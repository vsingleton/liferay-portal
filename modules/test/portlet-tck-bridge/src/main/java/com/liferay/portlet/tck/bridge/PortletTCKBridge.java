		...
		while ((System.currentTimeMillis() - startTime) < timeout) {
			...
			if (servletContext != null) {
				...
				while (attributeNames.hasMoreElements()) {
					...
					boolean beanPortletIdsFound = beanPortletIdsMatcher.find();
					...
					boolean pluginFound = pluginMatcher.find();

					if (beanPortletIdsFound || pluginFound) {
						if (_log.isInfoEnabled()) {
							_log.info(
			...
			Object comLiferayBeanPortletIds = servletContext.getAttribute(
				"com.liferay.beanPortletIds");
			...
			if (found) {
				if (comLiferayBeanPortletIds == null) {
					if (beanPortletIds == null) {
						if (pluginPortlets == null) {
							if (_log.isWarnEnabled()) {
								_log.warn(
									"_waitForDeployment: no bean portlet ids " +
										"or PLUGIN_PORTLETS attribute for " +
											servletContextName);
							}
